...
export class DataService {
  ...
  getAllIssues(): void {
    this.httpClient
      .get<Usuario[]>(`${this.API_URL}/getDatosUsuarios`)
      .subscribe({
        next: (data) => {
  ...
  addIssue(usuario: Usuario): void {
    this.httpClient
      .post(
        `${this.API_URL}/agregarUsuario?rut=${usuario.rut}&nombre=${usuario.nombre}&apellido=${usuario.apellido}&correo=${usuario.correo}&telefono=${usuario.telefono}&user=${usuario.user}&pass=${usuario.pass}`,
        usuario
      )
      .subscribe(
        (data) => {
  ...
  addRespuestas(
    ...
    this.httpClient
      .post(
        `${this.API_URL}/agregarRespuesta?analista=${usuario_id}&padron=${padron_id}&pregunta1=${preguntas[0]._id}&pregunta2=${preguntas[1]._id}&pregunta3=${preguntas[2]._id}&pregunta4=${preguntas[3]._id}&respuesta1=${respuestas[0]}&respuesta2=${respuestas[1]}&respuesta3=${respuestas[2]}&respuesta4=${respuestas[3]}&otro=${otro}&correo=${correo}`,
        'a'
      )
      .subscribe((data) => {
        console.log('Bakán');
  ...
  getCountRespuestasPorAnalista(): Observable<RespuestaXAnalista[]> {
    return this.httpClient.get<RespuestaXAnalista[]>(
      `${this.API_URL}/getCountRepuestasPorAnalista`
    );
  }
  ...
  getDatosPadron(analista: any): Observable<any[]> {
    return this.httpClient.get<any[]>(
      `${this.API_URL}/getDatosPadronPorAnalista?analista=${analista}`
    );
  }
  ...
  updateIssue(usuario: Usuario): void {
    this.httpClient
      .put(
        `${this.API_URL}/actualizarUsuario?_id=${usuario._id}&rut=${usuario.rut}&nombre=${usuario.nombre}&apellido=${usuario.apellido}&correo=${usuario.correo}&telefono=${usuario.telefono}&user=${usuario.user}&pass=${usuario.pass}`,
        null
      )
      .subscribe(
        (data) => {
  ...
  deleteIssue(rut: number): void {
    this.httpClient
      .delete(`${this.API_URL}/eliminarUsuarioPorRut?rut=${rut}`)
      .subscribe({
        next: (data) => {
          console.log(data);
          console.log('Bakán.');
  ...
  putAsignacion(analista: string, bodega: string) {
    this.httpClient
      .put(
        `${this.API_URL}/putAsignacion?analista=${analista}&bodega=${bodega}`,
        null
      )
      .subscribe({
        next: (data) => {
